...
export function NReceipt<TReceipt, TLine>({
  ...
  renderReceiptHeader, renderReceiptFooter, labels: labelsProp, documentLabels, ...documentProps
  ...
  const bodySection: NDocumentSection<TReceipt> = {
    ...
    render: (activeReceipt) => {
      ...
      return <Stack gap="5">
        {beforeLines}
        {lines.length === 0 ? <Text role="status" color="fg.muted">{labels.emptyLines}</Text> : <Stack gap="0">
          <Grid display={{ base: "none", md: "grid" }} gridTemplateColumns={getLineUnitAmount ? "minmax(12rem, 1fr) 7rem 9rem 9rem" : "minmax(12rem, 1fr) 7rem 9rem"} gap="3" px="3" pb="2" color="fg.muted" fontSize="xs" fontWeight="medium"><Text>{labels.item}</Text><Text textAlign="end">{labels.quantity}</Text>{getLineUnitAmount ? <Text textAlign="end">{labels.unitAmount}</Text> : null}<Text textAlign="end">{labels.lineTotal}</Text></Grid>
          <Stack as="ul" aria-label={labels.linesLabel} gap="0" m="0" p="0">{lines.map((line, index) => <Grid as="li" key={getLineId(line, index)} listStyleType="none" gridTemplateColumns={{ base: "minmax(0, 1fr) auto", md: getLineUnitAmount ? "minmax(12rem, 1fr) 7rem 9rem 9rem" : "minmax(12rem, 1fr) 7rem 9rem" }} gap="3" alignItems="center" py="3" px="3" borderTopWidth="1px" borderColor="border">
            <Box minW="0"><Box fontWeight="medium">{getLineLabel(line)}</Box>{getLineDescription ? <Box color="fg.muted" fontSize="sm">{getLineDescription(line)}</Box> : null}</Box>
            <Box textAlign="end"><Text display={{ base: "block", md: "none" }} color="fg.muted" fontSize="xs">{labels.quantity}</Text>{getLineQuantity?.(line) ?? 1}</Box>
            {getLineUnitAmount ? <Box display={{ base: "none", md: "block" }} textAlign="end">{presentAmount(getLineUnitAmount(line))}</Box> : null}
            <Box gridColumn={{ base: "1 / -1", md: "auto" }} textAlign="end" fontWeight="semibold"><Text display={{ base: "inline", md: "none" }} mr="2" color="fg.muted" fontSize="xs">{labels.lineTotal}</Text>{presentAmount(getLineTotal(line))}</Box>
          </Grid>)}</Stack>
        </Stack>}
        {afterLines}
        <Stack as="dl" aria-label={labels.summaryLabel} gap="2" m="0" ml={{ base: "0", md: "auto" }} width={{ base: "full", md: "22rem" }}>
          {summaryRows.map((row) => <Flex key={row.id} justify="space-between" gap="4"><Box as="dt" color="fg.muted">{row.label}</Box><Box as="dd" m="0">{presentAmount(row.amount)}</Box></Flex>)}
          <Flex justify="space-between" gap="4" pt="3" borderTopWidth="1px" borderColor="border"><Text as="dt" fontWeight="semibold">{labels.total}</Text><Box as="dd" m="0" fontSize="xl" fontWeight="bold">{presentAmount(getTotal(activeReceipt))}</Box></Flex>
        </Stack>
      </Stack>
    },
  }
