...
export function NBreadcrumbs({ items, maxItems = 4, separator = <ChevronRight aria-hidden="true" size={14} />, labels: labelsProp, unstyled = false, classNames, styles }: NBreadcrumbsProps) {
  ...
  return (
    <Box as="nav" aria-label={labels.navigationLabel} overflowX="auto" className={classNames?.root} css={styles?.root} data-scope="n-breadcrumbs" data-part="root">
      <Box as="ol" display="flex" alignItems="center" gap="2" listStyleType="none" m="0" p="0" whiteSpace="nowrap" fontSize={unstyled ? undefined : "sm"} className={classNames?.list} css={styles?.list} data-part="list">
        {visible.map((item, index) => (
          <Fragment key={item?.id ?? "overflow"}>
            {index > 0 ? <Box as="li" aria-hidden="true" color={unstyled ? undefined : "fg.muted"} className={classNames?.separator} css={styles?.separator} data-part="separator">{separator}</Box> : null}
            <Box as="li" className={classNames?.item} css={styles?.item} data-part="item">
