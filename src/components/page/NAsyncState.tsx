...
export function NAsyncState({ status, children, error, onRetry, loadingFallback, errorFallback, emptyFallback, skeletonLines = 4, minHeight = "12rem", labels: labelsProp }: NAsyncStateProps) {
  ...
  if (status === "loading") return loadingFallback ?? <Stack role="status" aria-label={labels.loading} minH={minHeight} justify="center" gap="3">{Array.from({ length: Math.max(1, skeletonLines) }, (_, index) => <Skeleton key={index} height={index === 0 ? "8" : "5"} width={index === skeletonLines - 1 ? "65%" : "full"} />)}</Stack>
  if (status === "empty") return emptyFallback ?? <NEmptyState title={labels.emptyTitle} description={labels.emptyDescription} />
  return errorFallback ?? <Center role="alert" minH={minHeight} p="6" borderWidth="1px" borderColor="border.error" rounded="lg" bg="bg.error"><Stack align="center" gap="3" textAlign="center"><Text color="fg.error" fontWeight="semibold">{labels.errorTitle}</Text><Box color="fg.error" fontSize="sm">{error ?? labels.errorDescription}</Box>{onRetry ? <Button size="sm" variant="outline" colorPalette="red" loading={retrying} onClick={() => void retry()}><RefreshCw aria-hidden="true" size={16} />{labels.retry}</Button> : null}</Stack></Center>
}
