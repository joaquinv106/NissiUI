...
export interface NSubscriptionGateProps { allowed: boolean; children: ReactNode; feature?: string; fallback?: ReactNode; onUpgrade?: () => void; labels?: Partial<NSubscriptionGateLabels> }
...
export interface NPlanComparisonProps { plans: NPlan[]; features: NPlanFeature[]; labels?: Partial<NPlanComparisonLabels> }
...
export interface NAuditLogProps { entries: NAuditEntry[]; title?: ReactNode; labels?: Omit<Partial<NAuditLogLabels>, "severity"> & { severity?: Partial<NAuditLogLabels["severity"]> }; "aria-label"?: string }

export interface NImpersonationBannerLabels { prefix: string; actorPrefix: string; exit: string; exiting: string }
export interface NImpersonationBannerProps { subject: ReactNode; actor?: ReactNode; onExit: () => void | Promise<void>; labels?: Partial<NImpersonationBannerLabels>; sticky?: boolean }
