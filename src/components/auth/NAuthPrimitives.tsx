...
import type { NAuthAlertProps, NAuthDividerProps, NAuthFooterProps, NAuthHeaderProps, NAuthLayoutProps, NPasswordFieldProps, NPasswordStrengthProps } from "./types"
import { getPasswordScore } from "./utils"

export function NAuthLayout({ variant = "centered", logo, brandName, title, subtitle, illustration, illustrationAlt = "", backgroundImage, children, footer, maxWidth = "28rem", reverse = false, containerProps, panelProps, unstyled = false, classNames, styles }: NAuthLayoutProps) {
  ...
  return (
    <Flex as="main" className={classNames?.root} css={styles?.root} data-scope="n-auth-layout" data-part="root" minH="100dvh" width="full" direction={{ base: "column", md: reverse ? "row-reverse" : "row" }} bg={unstyled ? undefined : "bg.subtle"} backgroundImage={backgroundImage ? `linear-gradient(color-mix(in srgb, var(--chakra-colors-bg) 25%, transparent), color-mix(in srgb, var(--chakra-colors-bg) 25%, transparent)), url(${backgroundImage})` : undefined} backgroundSize="cover" {...safeContainerProps}>
      {visual ? <Flex className={classNames?.visual} css={styles?.visual} data-part="visual" flex="1" minH={{ base: "14rem", md: "100dvh" }} p={{ base: "8", lg: "16" }} position="relative" overflow="hidden" align="center" justify="center" bg={unstyled ? undefined : "colorPalette.subtle"} color={unstyled ? undefined : "colorPalette.fg"}>
        ...
        {!unstyled ? <Box aria-hidden position="absolute" width="24rem" height="24rem" borderRadius="full" bg="colorPalette.muted" filter="blur(80px)" opacity="0.45" insetBlockEnd="-8rem" insetInlineStart="-6rem" /> : null}
      </Flex> : null}
...
export function NPasswordStrength({ value, minLength = 8, labels: labelsProp, disabled = false, showRequirements = true, unstyled = false, classNames, styles }: NPasswordStrengthProps) {
...
export const NPasswordField = forwardRef<HTMLInputElement, NPasswordFieldProps>(function NPasswordField({ label, helperText, errorText, showStrength = false, strengthLabels, strengthMinLength = 8, size = "md", required, disabled, value, defaultValue, onChange, autoComplete = "current-password", unstyled = false, classNames, styles, ...props }, ref) {
  ...
  return <Field.Root unstyled={unstyled} className={classNames?.root} css={styles?.root} data-scope="n-password-field" data-part="root" invalid={Boolean(errorText)} required={required} disabled={disabled}>
    ...
    {showStrength ? <Box className={classNames?.strength} css={styles?.strength} data-part="strength" width="full"><NPasswordStrength value={password} minLength={strengthMinLength} labels={strengthLabels} disabled={disabled} /></Box> : null}
  </Field.Root>
})
