...
import { NAuthAlert, NAuthDivider, NAuthFooter, NAuthHeader, NPasswordField } from "./NAuthPrimitives"
import { NAuthSocialButtons } from "./NAuthSocialButtons"
import type { NForgotPasswordProps, NLoginProps, NRegisterData, NRegisterFieldName, NRegisterProps, NResetPasswordProps, NVerifyEmailProps } from "./types"
...
function fieldNameLabel(name: NRegisterFieldName, labels: ReturnType<typeof resolveNAuthLabels>) {
  return name === "confirmPassword" ? labels.confirmPassword : labels[name as keyof typeof labels] as string
}

export function NLogin({ title = "Bienvenido nuevamente", subtitle = "Ingresa tus credenciales", loginWith = "email", allowRememberMe = false, allowForgotPassword = false, showRegisterLink = false, socialProviders = [], validation, defaultValues, onSubmit, onForgotPassword, onRegister, onSocialProviderClick, labels: labelsProp, loading = false, disabled = false, error, success, size = "md", colorPalette = "blue", unstyled = false, classNames, styles }: NLoginProps) {
  ...
  return <Stack className={classNames?.root} css={styles?.root} data-scope="n-login" data-part="root" gap="5">
    ...
    <Stack as="form" className={classNames?.form} css={styles?.form} data-part="form" gap="4" onSubmit={submit as never}>
      ...
      <NPasswordField name="password" label={labels.password} autoComplete="current-password" defaultValue={defaultValues?.password} size={size} required disabled={disabled || busy} errorText={errors.password} />
...
const defaultRegisterFields: readonly NRegisterFieldName[] = ["name", "email", "password", "confirmPassword"]
export function NRegister({ title = "Crea tu cuenta", subtitle = "Comienza en unos minutos", fields = defaultRegisterFields, customFields = [], socialProviders = [], showLoginLink = false, passwordMinLength = 8, onSubmit, onLogin, onSocialProviderClick, labels: labelsProp, loading = false, disabled = false, error, success, size = "md", colorPalette = "blue", classNames, styles }: NRegisterProps) {
...
export function NForgotPassword({ title = "Recupera tu contraseña", subtitle = "Te enviaremos instrucciones para volver a entrar", loginWith = "email", onSubmit, onBackToLogin, labels: labelsProp, loading = false, disabled = false, error, success, colorPalette = "blue" }: NForgotPasswordProps) {
...
export function NResetPassword({ token, title = "Crea una nueva contraseña", subtitle, passwordMinLength = 8, showStrength = true, onSubmit, labels: labelsProp, loading = false, disabled = false, error, success, colorPalette = "blue" }: NResetPasswordProps) {
  ...
  return <Stack data-scope="n-reset-password" gap="5"><NAuthHeader title={title} subtitle={subtitle} /><AuthState error={error} success={success} /><Stack as="form" gap="4" onSubmit={submit}><NPasswordField name="password" label={labels.newPassword} autoComplete="new-password" required disabled={disabled || loading} showStrength={showStrength} strengthMinLength={passwordMinLength} errorText={fieldError} /><NPasswordField name="confirmPassword" label={labels.confirmPassword} autoComplete="new-password" required disabled={disabled || loading} errorText={fieldError === labels.passwordsDoNotMatch ? fieldError : undefined} /><Button type="submit" colorPalette={colorPalette} loading={loading} disabled={disabled}>{labels.resetPassword}</Button></Stack></Stack>
}

export function NVerifyEmail({ email, title, subtitle, actionLabel, onVerify, onResend, labels: labelsProp, loading = false, disabled = false, error, success, colorPalette = "blue" }: NVerifyEmailProps) {
  const labels = useMemo(() => resolveNAuthLabels(labelsProp), [labelsProp])
